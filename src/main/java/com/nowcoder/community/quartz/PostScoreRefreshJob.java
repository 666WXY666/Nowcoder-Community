...
public class PostScoreRefreshJob implements Job, CommunityConstant {
    ...
    static {
        try {
            ...
            throw new RuntimeException("初始化网站成立时间失败!", e);
        }
    }
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        logger.info("[任务开始] 正在刷新帖子分数: " + size + "个");
    ...
    private void refresh(int postId) {
        ...
        if (post == null) {
            logger.error("[任务异常] 该帖子不存在: id = " + postId);
            return;
        } else if (post.getStatus() == 2) {
            logger.error("[任务异常] 该帖子已被删除 id = " + postId);
            return;
        }
        ...
        double score = Math.log10(Math.max(w, 1))
                + (double) (post.getCreateTime().getTime() - EPOCH.getTime())
                / (1000 * 3600 * 24);
        // 更新帖子分数
